...
type
  {**
    Класс конфигурации модуля экспорта в SQLite
...
begin
  ...
  FConnectionString := '';
  FDatabasePath := '';
  FDriver := ''; // Для SQLite драйвер не требуется
...
begin
  // Для SQLite используется прямое подключение через DatabasePath
  // Эта функция сохранена для совместимости, но не используется
