...
uses
  ...
  uzvsqlite_parse_setcolumn, uzvsqlite_parse_keycolumn, uzvsqlite_parse_const;
...
begin
  ...
  begin
    programlog.LogOutFormatStr(
      'uzvsqlite: Инструкция tTable без имени таблицы - пропускается',
      [],
      LM_Info
    );
    Exit;
  ...
  if AInstruction.TargetTable <> '' then
    programlog.LogOutFormatStr(
      'uzvsqlite: Переопределение целевой таблицы: %s -> %s',
...
begin
  ...
  begin
    programlog.LogOutFormatStr(
      'uzvsqlite: Инструкция typeData без типа данных - пропускается',
      [],
      LM_Info
    );
    Exit;
...
begin
  ...
  programlog.LogOutFormatStr(
    'uzvsqlite: Начало парсинга таблицы: %s',
    [ATableName],
    LM_Info
  );
  ...
  try
    ...
    begin
      ...
      case instructionType of
        ...
        itSetColumn:
          uzvsqlite_parse_setcolumn.ParseSetColumnInstruction(Result, col3, col4, col5);

        itKeyColumn:
          uzvsqlite_parse_keycolumn.ParseKeyColumnInstruction(Result, ADataset);

        itConst:
          uzvsqlite_parse_const.ParseConstInstruction(Result, col3, col4);

        itUnknown:
          programlog.LogOutFormatStr(
            'uzvsqlite: Неизвестная инструкция "%s" в строке %d - пропускается',
            [col2, rowCount],
            LM_Info
          );
      end;
    ...
    begin
      programlog.LogOutFormatStr(
      'uzvsqlite: Не найдена инструкция tTable - целевая таблица не задана',
      [],
      LM_Info
    );
    ...
    begin
      programlog.LogOutFormatStr(
      'uzvsqlite: Не найдено ни одной инструкции setcolumn',
      [],
      LM_Info
    );
    end;
    ...
    programlog.LogOutFormatStr(
      'uzvsqlite: Парсинг завершен: %d строк, %d маппингов, %d ключевых колонок',
