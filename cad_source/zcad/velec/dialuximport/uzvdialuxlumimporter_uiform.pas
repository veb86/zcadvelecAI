...
begin
  // ВАЖНО: Не выполнять инициализацию управляемых типов (строк) в OnInitNode!
  //
  // VirtualStringTree автоматически инициализирует память узла нулями при
  // его создании. Попытка присвоения значений управляемым типам (строкам)
  // в этом обработчике может привести к ошибкам доступа к памяти, так как
  // менеджер памяти еще не завершил полную инициализацию структуры данных узла.
  //
  // Фактические значения полей NodeData устанавливаются после создания узла
  // в методе CreateLightNode, когда узел уже полностью инициализирован компонентом.
  //
  // Все поля записи TLightMappingNodeData будут автоматически установлены в
  // значения по умолчанию: пустые строки ('') для string, нули для числовых типов.
end;
