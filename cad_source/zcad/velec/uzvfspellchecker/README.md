...
- **uzvfspelldata.pas** - структуры данных для хранения информации об ошибках
  ...
  - Поля TSpellError:
    ...
    - EntityPtr: Pointer - текстовый примитив, если ошибка найдена в чертеже
...
**Верхняя панель ошибок (TLazVirtualStringTree):**
- Отображает список всех найденных ошибок отдельной строкой для каждого слова
- Колонки:
  - Слово с ошибкой
- Сортировка по умолчанию: по порядку появления в тексте
- При выборе элемента:
  - Обновляется нижняя панель с вариантами исправления
  - Обновляется нижняя панель с предложением

**Нижняя панель вариантов (TLazVirtualStringTree):**
- Отображает варианты исправления для выбранной ошибки
- Колонки:
  - Вариант исправления
- Пустая, если ошибка не выбрана
- Кнопка "Применить" или двойной клик по варианту применяет исправление
...
|  [Обновить] [Применить]                           |
+--------------------------------------------------+
|  TLazVirtualStringTree (Ошибки, 70%)              |
|  - слово1                                         |
|  - слово2                                         |
|  - слово3                                         |
+--------------------------------------------------+
|  TLazVirtualStringTree (Варианты исправления, 30%)|
|  - вариант1                                       |
|  - вариант2                                       |
+--------------------------------------------------+
...
2. Отображение списка ошибок в верхнем дереве без объединения повторов
3. Получение вариантов исправления от Hunspell
4. Отображение вариантов в нижнем дереве при выборе ошибки
5. Отображение сообщения с ошибкой и контекстом в нижней метке
6. Обновление списка ошибок по кнопке "Обновить"
7. Применение выбранного варианта кнопкой или двойным щелчком

#### 6.2. Дополнительная функциональность
1. Выделение ошибочного слова в предложении цветом
2. Фильтрация ошибок (все/только неизвестные слова)
3. Экспорт списка ошибок
...
├── uzvfspelluserdict.pas        # Работа с пользовательским словарём (добавление/удаление/чтение слов)
├── uzvfspellform.pas            # Код формы проверки орфографии
├── uzvfspellform.lfm            # Визуальная часть формы проверки орфографии
├── uzvfuserdictform.pas         # Код формы управления пользовательским словарём (issue #1361)
└── uzvfuserdictform.lfm         # Визуальная часть формы управления словарём
```

#### Управление пользовательским словарём (issue #1361)

Команда `SpellUserDict` открывает форму `uzvfuserdict` управления
пользовательским словарём. На форме:
- быстрый список добавленных слов (тот же элемент `TLazVirtualStringTree`,
  что и список слов с ошибками);
- поле поиска для быстрой фильтрации длинного списка по подстроке
  (без учёта регистра);
- кнопка «Удалить» удаляет выбранное слово из словаря и перезагружает
  проверку орфографии, после чего слово снова считается ошибкой.
