...
uses
  ...
  Dialogs, ExtCtrls, ActnList, ComCtrls, Windows,fgl,odbcconn,
...
type
  ...
  PVstDevNodeData = ^TVstDevNodeData;
  TVstDevNodeData = record
    DeviceName: string;
    HDName: string;
    HDGroup: string;
  ...
  try
    ...
    try
      ...
      vstDev.TreeOptions.PaintOptions :=
        vstDev.TreeOptions.PaintOptions + [toShowRoot, toShowTreeLines];
    ...
    on E: Exception do
      ShowMessage('Ошибка создания vstDev: ' + E.Message);
  end;
end;
...
begin
  try
    ...
    try
      ...
      try
        ...
        begin
          ...
          NodeData^.DeviceName := writeQuery.FieldByName('devname').AsString;
          NodeData^.HDName := writeQuery.FieldByName('hdname').AsString;
          NodeData^.HDGroup := writeQuery.FieldByName('hdgroup').AsString;
          NodeData^.IsShowButton := False;
          NodeData^.IsEditButton := False;
    ...
    on E: Exception do
      ShowMessage('Ошибка заполнения vstDev: ' + E.Message);
  end;
end;
...
begin
  ...
  case Column of
    0: CellText := 'Показать';
    1: CellText := NodeData^.DeviceName;
...
begin
  ...
  Tree.GetHitTestInfoAt(Tree.ScreenToClient(Mouse.CursorPos).X,
                        Tree.ScreenToClient(Mouse.CursorPos).Y,
                        True, HitInfo);
  ...
  if HitInfo.HitColumn = 0 then
    ShowMessage('devname: ' + NodeData^.DeviceName)
