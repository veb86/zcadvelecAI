...
begin
  ...
  det := MatrixDeterminant(t_matrix);
  zcUI.TextMessage(
     Format('Матрица трансформации: det=%.6f', [det])
    ,TMWOHistoryOut);

  precalc;
  { Проверка на зеркалирование: det < 0 означает отражение }
  { При отражении нужно поменять начальную и конечную точки местами }
  if det < 0 then begin
    sav:=q2;
    eav:=q0;
    zcUI.TextMessage(
      'Определитель < 0: зеркалирование, меняем местами начальную и конечную точки'
    ,TMWOHistoryOut);
  end else begin
    sav:=q0;
    eav:=q2;
  ...
  { (которая получилась ПОСЛЕ трансформации) для правильного расчета углов }
  ...
  { (относительно новой оси X, которая получилась после трансформации) }
